...
Menu.SetValueName(Tinker.Version, 1, "4.3.3 (revorked)")
...
Tinker.Items[13] =  "Eul's on Enemy"
Tinker.Items[14] =  "Eul's on Self"
Tinker.Items[15] =  "Blink to Mouse"
Tinker.Items[16] =  "Blink to Base"
Tinker.Items[17] =  "Lotus"
Tinker.Items[18] =  "Medallion of Courage"
Tinker.Items[19] =  "Solar Crest"
Tinker.Items[20] =  "Delay (0.1s)"
-- Hero Skills
...
function CalculateTotalDMG()
  ...
  local rearm = NPC.GetAbility(Tinker.Hero, 'tinker_rearm')
	...
	if Ability.GetLevel(uniqLaserBonus) == 1 then
		Tinker.TotalPureDamage = Tinker.TotalPureDamage + 80
	end
...
function Hex()
	...
	then 
		...
		Tinker.MoveTo(Tinker.NearestEnemyHeros)
...
function Matrix()
    local abilityMatrix = NPC.GetAbilityByIndex(Tinker.Hero, 2)
	if not abilityMatrix then return end

	if	Ability.IsCastable(abilityMatrix, Tinker.ManaPoint)
		and Tinker.LastCastAbility ~= abilityMatrix
	then 
		Ability.CastTarget(abilityMatrix, Tinker.Hero) 
